...
async def send_course_outcomes_message(course_id: str, thread_id: str, message: str, user_id: str) -> str:
    ...
    try:
        ...
        chat_history = await get_course_outcomes_history(course_id, thread_id)
        
        # Get current resource status for system prompt
        course_resources = storage_service.get_resources(course_id, user_id=user_id)
        asset_resources = storage_service.get_resources(course_id, thread_id=thread_id, user_id=user_id)
        all_resources = course_resources + asset_resources
        ...
        # Create system prompt with check-in/check-out information
        system_prompt = f"""You are a Course Outcomes Generator assistant. You help generate detailed course outcomes based on the provided context.

Available files for reference: {', '.join(checked_in_files) if checked_in_files else 'None'}

Files that are currently checked out and should not be referenced: {', '.join(checked_out_files) if checked_out_files else 'None'}

Please generate course outcomes based on the available files and user input. If asked about checked-out files, inform the user that those files are currently unavailable."""
        ...
        # Add system message with current resource status
        client.beta.threads.messages.create(
