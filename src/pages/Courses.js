...
export default function Courses() {
  ...
  const handleGetStarted = async () => {
    try {
      // You may want to collect year/level from the user; for now, use defaults
      ...
      localStorage.setItem("currentCourseId", newCourse.courseId || newCourse.id || newCourse._id);
      console.log("Created course and stored ID:", newCourse.courseId || newCourse.id || newCourse._id);
      setShowModal(false);
      setCourseName("");
      setCourseDesc("");
      navigate("/dashboard");
  ...
  const handleCardClick = (course) => {
    ...
    localStorage.setItem("currentCourseId", course.courseId || course.id || course._id);
    console.log("Selected course and stored ID:", course.courseId || course.id || course._id);
    navigate("/dashboard");
  };

  const handleDeleteCourse = async (idx, e) => {
    e.stopPropagation();
    if (window.confirm("Are you sure you want to delete this course?")) {
      try {
        const courseId = courses[idx].courseId || courses[idx].id || courses[idx]._id;
        await deleteCourse(courseId);
        setCourses(courses => courses.filter((_, i) => i !== idx));
      } catch (e) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: courses.length === 0 ? "center" : "flex-start", justifyContent: courses.length === 0 ? "center" : "flex-start", width: "100%" }}>
        {loading ? (
          <div style={{ textAlign: "center", width: "100%", marginTop: 40 }}>Loading...</div>
        ) : courses.length === 0 ? (
