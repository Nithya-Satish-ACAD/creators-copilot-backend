...
export default function Studio() {
  ...
  const handleDownload = (content, timestamp) => {
    // Get course name and asset type for better PDF naming
    const courseName = localStorage.getItem("currentCourseTitle") || "Unknown Course";
    const assetType = option === "brainstorm" ? "Brainstorm" : "Chat";
    ...
    const filename = `${assetType.toLowerCase()}-${courseName.toLowerCase().replace(/\s+/g, '-')}-${timestampStr}.pdf`;
  ...
  const handleSaveToResources = async (content, timestamp, messageIndex) => {
    ...
    try {
      ...
      showSuccess("Chat response saved to resources successfully!");
    } catch (error) {
      ...
      <div style={{ display: "flex", maxWidth: 1200, margin: "24px auto 0 auto", gap: 18, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 3, background: "#fff", borderRadius: 14, minHeight: 520, display: "flex", flexDirection: "column", boxShadow: "0 1px 4px #0001" }}>
          ...
          <div style={{ flex: 1, padding: 18, overflowY: "auto" }}>
            {!isLoading && messages.length === 0 && option === "brainstorm" && (
              <div style={{ textAlign: "center", color: "#666", padding: "20px" }}>
                <div style={{ fontSize: 24, marginBottom: 10 }}></div>
                <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>Brainstorm Session Ready</div>
                <div style={{ fontSize: 14, marginBottom: 4 }}>The system is ready to help you brainstorm ideas!</div>
                <div style={{ fontSize: 12, color: "#888", marginBottom: 8 }}>Start typing to begin your brainstorming session</div>
                <div style={{ fontSize: 11, color: "#999" }}>
                  Quick actions: "help me brainstorm", "generate ideas", "explore topics"
                </div>
              </div>
            )}
            {isLoading && !messages.length && (
              <div style={{ textAlign: "center", color: "#666", padding: "20px" }}>
                <div style={{ fontSize: 24, marginBottom: 10 }}></div>
                <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>Setting up Brainstorm Session</div>
