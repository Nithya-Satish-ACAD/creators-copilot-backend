...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await new Promise((resolve) => setTimeout(() => resolve(login(username, password)), 800));
      if (result) {
        navigate("/courses");
      } else {
    ...
    <div style={{ maxWidth: 400, margin: "2rem auto", padding: 24, border: "1px solid #eee", borderRadius: 8 }}>
      ...
      {loading ? (
        ...
        <form onSubmit={handleSubmit}>
          <Input label="Username" value={username} onChange={e => setUsername(e.target.value)} required disabled={loading} />
          <Input label="Password" type="password" value={password} onChange={e => setPassword(e.target.value)} required disabled={loading} />
